...
class UnitBreaker(object):
    ...
    def find_similar_units(gr, tvd, boundary_flags, lithofacies, gr_shape_code, thickness,
                           ...
                           max_depth, min_depth=0, unit_index=None, return_unit_index=False,
                            ...
                            if lithofacies[current_unit_id] == lithofacies[comparison_unit_id] and \
                                ...
                                if thickness[current_unit_id] * 0.5 < thickness[comparison_unit_id] < \
                                    ...
                                    if abs(mean_unit[current_unit_id] - mean_unit[comparison_unit_id]) < mean_threshold:
                                        score += weights['mean']
                                    if abs(variance_1[current_unit_id] - variance_1[
                                        comparison_unit_id]) < variance_1_threshold:
                                        score += weights['variance1']
                                    if abs(variance_2[current_unit_id] - variance_2[
                                        comparison_unit_id]) < variance_2_threshold:
        ...
        similar_unit_list_by_samples = []

        tmp = 0
        for flag in boundary_flags:
            similar_unit_list_by_samples.append(similar_unit_list[tmp])
            if int(flag) == 1:
                tmp += 1

        if return_unit_index:
            return unit_index, number_of_similar_pattern, similar_unit_list_by_samples
        return number_of_similar_pattern, similar_unit_list_by_samples
